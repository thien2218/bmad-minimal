...
async function updatePackage(workspacePath, options) {
  ...
  if (!await fs.pathExists(bmadCorePath)) {
    throw new Error(`'${workspacePath}' does not appear to be a BMAD workspace (missing .bmad-core)`);
  }
  ...
  console.log(chalk.gray(`Current workspace version: ${currentVersion || 'unknown'}`));
...
async function readExistingConfig(bmadCorePath) {
  ...
  if (await fs.pathExists(configPath)) {
    try {
      ...
      return {
        includePlanning: await fs.pathExists(path.join(path.dirname(bmadCorePath), 'planning')),
        folders: {
      ...
      console.warn(chalk.yellow('⚠️ Could not read existing config, using defaults'));
    }
  }
...
async function preserveUserCustomizations(workspacePath) {
  const techPrefsPath = path.join(workspacePath, '.bmad-core', 'technical-preferences.md');
  ...
  if (await fs.pathExists(techPrefsPath)) {
    try {
      ...
      if (!content.includes('Auto-generated by BMAD Minimal CLI')) {
        ...
        console.log(chalk.yellow(`⚠️ Technical preferences have been customized`));
        console.log(chalk.gray(`   Original backed up to: ${path.basename(backupPath)}`));
        console.log(chalk.gray('   Please merge your changes manually if needed.'));
      }
    } catch (error) {
...
`;
  ...
  if (!await fs.pathExists(gitignorePath)) {
    ...
    if (existingContent.includes('# BMAD Core files') && existingContent.length < 500) {
      await fs.writeFile(gitignorePath, expectedContent);
    }
  }
}
