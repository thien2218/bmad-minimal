...
async function setupWorkspace(targetPath, options) {
	...
	if (await fs.pathExists(workspacePath)) {
		...
		try {
			...
			throw new Error(
				`Failed to remove existing workspace: ${error.message}`
			);
		}
	}
	...
	try {
		...
		config = options.yes
			? getDefaultConfig()
			: await promptUserConfig(targetPath);
		...
		if (await fs.pathExists(workspacePath)) {
			try {
				...
				console.warn(
					chalk.yellow(
						`⚠️ Could not clean up workspace: ${cleanupError.message}`
					)
				);
			}
		}
	...
	const questions = [
		{
			...
			message:
				"Do you want to include planning files? (recommended for web-based planning)",
			default: true,
		},
		{
			type: "input",
...
async function createWorkspaceStructure(workspacePath, config) {
	...
	await fs.ensureDir(
		path.join(bmadCorePath, config.folders.docs, "architecture")
	);
}
...
`;
		...
		console.warn(
			chalk.yellow(
				"⚠️ Could not initialize git repository. You can do this manually later."
			)
		);
	}
}
