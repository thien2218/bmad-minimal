...
function generateTechPreferences(projectInfo, config) {
	const languages =
		projectInfo.languages && projectInfo.languages.length > 0
			? projectInfo.languages.join(", ")
			: "TBD";

	const runtime = projectInfo.runtime || "TBD";
	const frameworks =
		projectInfo.frameworks && projectInfo.frameworks.length > 0
			? projectInfo.frameworks
			: [];

	const packageManager =
		projectInfo.type === "nodejs" ? projectInfo.packageManager : "N/A";
