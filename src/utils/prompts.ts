...
export function buildCodingStandardsPrompt({
	...
	if (backendDir) contextLines.push(`- Backend directory: ${backendDir}`);
	if (frontendDir) contextLines.push(`- Frontend directory: ${frontendDir}`);
	contextLines.push(`- Docs directory: ${docsDir}`);

	return (
		`Task: Create or update ${displayPath} with the team's coding conventions using the lean template headings (naming, files & directories, imports/exports, error handling, logging, testing, security & privacy, Git/PR, and the short review checklist).` +
		`\n\nInstructions:` +
